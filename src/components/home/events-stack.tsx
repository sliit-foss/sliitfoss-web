    ...
    <section className="py-20 md:py-32 px-6 bg-[#fafafa]">
      <div className="max-w-5xl mx-auto">
        <div className="mb-16 md:mb-24">
          <div className="font-mono text-[0.65rem] text-[#999] uppercase tracking-wider mb-4">What&apos;s Next</div>
        ...
        <StackCards>
          {events.map((event) => (
            <Link key={event.slug} href={`/events/${event.slug}`}>
              <div
                className={`relative rounded-3xl overflow-hidden ${event.color.bg} border border-black/6 min-h-[50vh] md:min-h-[60vh] p-8 md:p-12 flex flex-col justify-between group cursor-pointer transition-all duration-500 ${event.color.borderHover} hover:shadow-xl shadow-[0_4px_20px_rgba(0,0,0,0.06)]`}
              >
                <div
                ...
                <div className="relative z-10 flex items-end justify-between">
                  <p className="text-sm md:text-base text-[#666] max-w-lg leading-relaxed">{event.shortDescription}</p>
                  <span
                    className={`text-sm ${event.color.accent} opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 shrink-0 ml-6`}
                  >
                    Learn more →
                  </span>
                </div>
